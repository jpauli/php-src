...
ZEND_API void zend_check_magic_method_implementation(const zend_class_entry *ce, const zend_function *fptr, int error_type TSRMLS_DC) /* {{{ */
{
	...
	if (name_len == sizeof(ZEND_DESTRUCTOR_FUNC_STR_NAME) - 1 && !memcmp(lcname, ZEND_DESTRUCTOR_FUNC_STR_NAME, sizeof(ZEND_DESTRUCTOR_FUNC_STR_NAME)) && fptr->common.num_args != 0) {
		zend_error(error_type, "Destructor %s::%s() cannot take arguments", ce->name, ZEND_DESTRUCTOR_FUNC_STR_NAME);
	} else if (name_len == sizeof(ZEND_CLONE_FUNC_STR_NAME) - 1 && !memcmp(lcname, ZEND_CLONE_FUNC_STR_NAME, sizeof(ZEND_CLONE_FUNC_STR_NAME)) && fptr->common.num_args != 0) {
		zend_error(error_type, "Method %s::%s() cannot accept any arguments", ce->name, ZEND_CLONE_FUNC_STR_NAME);
	} else if (name_len == sizeof(ZEND_GET_FUNC_STR_NAME) - 1 && !memcmp(lcname, ZEND_GET_FUNC_STR_NAME, sizeof(ZEND_GET_FUNC_STR_NAME))) {
		if (fptr->common.num_args != 1) {
			zend_error(error_type, "Method %s::%s() must take exactly 1 argument", ce->name, ZEND_GET_FUNC_STR_NAME);
		} else if (ARG_SHOULD_BE_SENT_BY_REF(fptr, 1)) {
			zend_error(error_type, "Method %s::%s() cannot take arguments by reference", ce->name, ZEND_GET_FUNC_STR_NAME);
		}
	} else if (name_len == sizeof(ZEND_SET_FUNC_STR_NAME) - 1 && !memcmp(lcname, ZEND_SET_FUNC_STR_NAME, sizeof(ZEND_SET_FUNC_STR_NAME))) {
		if (fptr->common.num_args != 2) {
			zend_error(error_type, "Method %s::%s() must take exactly 2 arguments", ce->name, ZEND_SET_FUNC_STR_NAME);
		} else if (ARG_SHOULD_BE_SENT_BY_REF(fptr, 1) || ARG_SHOULD_BE_SENT_BY_REF(fptr, 2)) {
			zend_error(error_type, "Method %s::%s() cannot take arguments by reference", ce->name, ZEND_SET_FUNC_STR_NAME);
		}
	} else if (name_len == sizeof(ZEND_UNSET_FUNC_STR_NAME) - 1 && !memcmp(lcname, ZEND_UNSET_FUNC_STR_NAME, sizeof(ZEND_UNSET_FUNC_STR_NAME))) {
		if (fptr->common.num_args != 1) {
			zend_error(error_type, "Method %s::%s() must take exactly 1 argument", ce->name, ZEND_UNSET_FUNC_STR_NAME);
		} else if (ARG_SHOULD_BE_SENT_BY_REF(fptr, 1)) {
			zend_error(error_type, "Method %s::%s() cannot take arguments by reference", ce->name, ZEND_UNSET_FUNC_STR_NAME);
		}
	} else if (name_len == sizeof(ZEND_ISSET_FUNC_STR_NAME) - 1 && !memcmp(lcname, ZEND_ISSET_FUNC_STR_NAME, sizeof(ZEND_ISSET_FUNC_STR_NAME))) {
		if (fptr->common.num_args != 1) {
			zend_error(error_type, "Method %s::%s() must take exactly 1 argument", ce->name, ZEND_ISSET_FUNC_STR_NAME);
		} else if (ARG_SHOULD_BE_SENT_BY_REF(fptr, 1)) {
			zend_error(error_type, "Method %s::%s() cannot take arguments by reference", ce->name, ZEND_ISSET_FUNC_STR_NAME);
		}
	} else if (name_len == sizeof(ZEND_CALL_FUNC_STR_NAME) - 1 && !memcmp(lcname, ZEND_CALL_FUNC_STR_NAME, sizeof(ZEND_CALL_FUNC_STR_NAME))) {
		if (fptr->common.num_args != 2) {
			zend_error(error_type, "Method %s::%s() must take exactly 2 arguments", ce->name, ZEND_CALL_FUNC_STR_NAME);
		} else if (ARG_SHOULD_BE_SENT_BY_REF(fptr, 1) || ARG_SHOULD_BE_SENT_BY_REF(fptr, 2)) {
			zend_error(error_type, "Method %s::%s() cannot take arguments by reference", ce->name, ZEND_CALL_FUNC_STR_NAME);
		}
	} else if (name_len == sizeof(ZEND_CALLSTATIC_FUNC_STR_NAME) - 1 &&
		!memcmp(lcname, ZEND_CALLSTATIC_FUNC_STR_NAME, sizeof(ZEND_CALLSTATIC_FUNC_STR_NAME)-1)
	) {
		if (fptr->common.num_args != 2) {
			zend_error(error_type, "Method %s::%s() must take exactly 2 arguments", ce->name, ZEND_CALLSTATIC_FUNC_STR_NAME);
		} else if (ARG_SHOULD_BE_SENT_BY_REF(fptr, 1) || ARG_SHOULD_BE_SENT_BY_REF(fptr, 2)) {
			zend_error(error_type, "Method %s::%s() cannot take arguments by reference", ce->name, ZEND_CALLSTATIC_FUNC_STR_NAME);
		}
 	} else if (name_len == sizeof(ZEND_TOSTRING_FUNC_STR_NAME) - 1 &&
 		!memcmp(lcname, ZEND_TOSTRING_FUNC_STR_NAME, sizeof(ZEND_TOSTRING_FUNC_STR_NAME)-1) && fptr->common.num_args != 0
	) {
		zend_error(error_type, "Method %s::%s() cannot take arguments", ce->name, ZEND_TOSTRING_FUNC_STR_NAME);
	}
}
/* }}} */
	...
	while (ptr->fname) {
		...
		if (scope) {
			...
			if ((fname_len == class_name_len) && !ctor && !memcmp(lowercase_name, lc_class_name, class_name_len+1)) {
				ctor = reg_function;
			} else if ((fname_len == sizeof(ZEND_CONSTRUCTOR_FUNC_STR_NAME)-1) && !memcmp(lowercase_name, ZEND_CONSTRUCTOR_FUNC_STR_NAME, sizeof(ZEND_CONSTRUCTOR_FUNC_STR_NAME))) {
				ctor = reg_function;
			} else if ((fname_len == sizeof(ZEND_DESTRUCTOR_FUNC_STR_NAME)-1) && !memcmp(lowercase_name, ZEND_DESTRUCTOR_FUNC_STR_NAME, sizeof(ZEND_DESTRUCTOR_FUNC_STR_NAME))) {
				dtor = reg_function;
				if (internal_function->num_args) {
					zend_error(error_type, "Destructor %s::%s() cannot take arguments", scope->name, ptr->fname);
				}
			} else if ((fname_len == sizeof(ZEND_CLONE_FUNC_STR_NAME)-1) && !memcmp(lowercase_name, ZEND_CLONE_FUNC_STR_NAME, sizeof(ZEND_CLONE_FUNC_STR_NAME))) {
				clone = reg_function;
			} else if ((fname_len == sizeof(ZEND_CALL_FUNC_STR_NAME)-1) && !memcmp(lowercase_name, ZEND_CALL_FUNC_STR_NAME, sizeof(ZEND_CALL_FUNC_STR_NAME))) {
				__call = reg_function;
			} else if ((fname_len == sizeof(ZEND_CALLSTATIC_FUNC_STR_NAME)-1) && !memcmp(lowercase_name, ZEND_CALLSTATIC_FUNC_STR_NAME, sizeof(ZEND_CALLSTATIC_FUNC_STR_NAME))) {
				__callstatic = reg_function;
			} else if ((fname_len == sizeof(ZEND_TOSTRING_FUNC_STR_NAME)-1) && !memcmp(lowercase_name, ZEND_TOSTRING_FUNC_STR_NAME, sizeof(ZEND_TOSTRING_FUNC_STR_NAME))) {
				__tostring = reg_function;
			} else if ((fname_len == sizeof(ZEND_GET_FUNC_STR_NAME)-1) && !memcmp(lowercase_name, ZEND_GET_FUNC_STR_NAME, sizeof(ZEND_GET_FUNC_STR_NAME))) {
				__get = reg_function;
			} else if ((fname_len == sizeof(ZEND_SET_FUNC_STR_NAME)-1) && !memcmp(lowercase_name, ZEND_SET_FUNC_STR_NAME, sizeof(ZEND_SET_FUNC_STR_NAME))) {
				__set = reg_function;
			} else if ((fname_len == sizeof(ZEND_UNSET_FUNC_STR_NAME)-1) && !memcmp(lowercase_name, ZEND_UNSET_FUNC_STR_NAME, sizeof(ZEND_UNSET_FUNC_STR_NAME))) {
				__unset = reg_function;
			} else if ((fname_len == sizeof(ZEND_ISSET_FUNC_STR_NAME)-1) && !memcmp(lowercase_name, ZEND_ISSET_FUNC_STR_NAME, sizeof(ZEND_ISSET_FUNC_STR_NAME))) {
...
static int zend_is_callable_check_func(int check_flags, zval *callable, zend_fcall_info_cache *fcc, int strict_class, char **error TSRMLS_DC) /* {{{ */
{
	...
	if (strict_class &&
	    fcc->calling_scope &&
	    mlen == sizeof(ZEND_CONSTRUCTOR_FUNC_STR_NAME)-1 &&
	    !memcmp(lmname, ZEND_CONSTRUCTOR_FUNC_STR_NAME, sizeof(ZEND_CONSTRUCTOR_FUNC_STR_NAME))) {
