...
#define ELSE_CHECK_VISIBILITY(METHOD, VISIBILITY_CHECK, VERB) else CHECK_VISIBILITY(METHOD, VISIBILITY_CHECK, VERB)

#define ELSE_CHECK_METHOD_VISIBILITY(METHOD, VISIBILITY_CHECK, VERB) else if ((name_len == sizeof(ZEND_STRINGIFY_MACRO(METHOD))-1) && (!memcmp(lcname, ZEND_STRINGIFY_MACRO(METHOD), sizeof(ZEND_STRINGIFY_MACRO(METHOD))-1))) { \
			if (VISIBILITY_CHECK) { \
				METHOD_VISIBILITY_ERROR(METHOD, VERB) \
			} \
...
#define ELSE_ADD_MAGIC_METHOD(METHOD) else ADD_MAGIC_METHOD(METHOD)
...
void zend_do_begin_function_declaration(znode *function_token, znode *function_name, int is_method, int return_reference, znode *fn_flags_znode TSRMLS_DC) /* {{{ */
{
	...
	if (is_method) {
		...
		if (CG(active_class_entry)->ce_flags & ZEND_ACC_INTERFACE) {
			...
			ELSE_CHECK_VISIBILITY(ZEND_CALLSTATIC_FUNC_NAME, (fn_flags & (ZEND_ACC_PPP_MASK ^ ZEND_ACC_PUBLIC)) || (fn_flags & ZEND_ACC_STATIC) == 0 , VISIBILITY_ERROR_MUST)
			ELSE_CHECK_VISIBILITY(ZEND_GET_FUNC_NAME, DEFAULT_VISIBILITY_CHECK, VISIBILITY_ERROR_CAN_NOT)
			ELSE_CHECK_VISIBILITY(ZEND_SET_FUNC_NAME, DEFAULT_VISIBILITY_CHECK, VISIBILITY_ERROR_CAN_NOT)
			ELSE_CHECK_VISIBILITY(ZEND_UNSET_FUNC_NAME, DEFAULT_VISIBILITY_CHECK, VISIBILITY_ERROR_CAN_NOT)
			ELSE_CHECK_VISIBILITY(ZEND_ISSET_FUNC_NAME, DEFAULT_VISIBILITY_CHECK, VISIBILITY_ERROR_CAN_NOT)
			ELSE_CHECK_VISIBILITY(ZEND_TOSTRING_FUNC_NAME, DEFAULT_VISIBILITY_CHECK, VISIBILITY_ERROR_CAN_NOT)
			ELSE_CHECK_VISIBILITY(ZEND_INVOKE_FUNC_NAME, DEFAULT_VISIBILITY_CHECK, VISIBILITY_ERROR_CAN_NOT)
			...
			ELSE_CHECK_METHOD_VISIBILITY(ZEND_CALL_FUNC_NAME, DEFAULT_VISIBILITY_CHECK, VISIBILITY_ERROR_CAN_NOT)
			ELSE_CHECK_METHOD_VISIBILITY(ZEND_CALLSTATIC_FUNC_NAME, (fn_flags & (ZEND_ACC_PPP_MASK ^ ZEND_ACC_PUBLIC)) || (fn_flags & ZEND_ACC_STATIC) == 0 , VISIBILITY_ERROR_MUST)
			ELSE_CHECK_METHOD_VISIBILITY(ZEND_GET_FUNC_NAME, DEFAULT_VISIBILITY_CHECK, VISIBILITY_ERROR_CAN_NOT)
			ELSE_CHECK_METHOD_VISIBILITY(ZEND_SET_FUNC_NAME, DEFAULT_VISIBILITY_CHECK, VISIBILITY_ERROR_CAN_NOT)
			ELSE_CHECK_METHOD_VISIBILITY(ZEND_UNSET_FUNC_NAME, DEFAULT_VISIBILITY_CHECK, VISIBILITY_ERROR_CAN_NOT)
...
static void zend_add_magic_methods(zend_class_entry* ce, const char* mname, uint mname_len, zend_function* fe TSRMLS_DC) /* {{{ */
{
	...
	ELSE_ADD_MAGIC_METHOD(ZEND_GET_FUNC_NAME)
	ELSE_ADD_MAGIC_METHOD(ZEND_SET_FUNC_NAME)
	ELSE_ADD_MAGIC_METHOD(ZEND_CALL_FUNC_NAME)
	ELSE_ADD_MAGIC_METHOD(ZEND_UNSET_FUNC_NAME)
	ELSE_ADD_MAGIC_METHOD(ZEND_ISSET_FUNC_NAME)
	ELSE_ADD_MAGIC_METHOD(ZEND_CALLSTATIC_FUNC_NAME)
	ELSE_ADD_MAGIC_METHOD(ZEND_TOSTRING_FUNC_NAME)
